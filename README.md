...
- *Highlights:* Subscribes to arbitrary MQTT topic filters, stores raw MQTT envelopes, preserves JSON payloads, and optionally flattens JSON scalar leaves for indexed querying
...
Because these are fully featured tools, they can also be used as foundations for developing specialized MQTT applications. For durable persistence, *MQTTStore* provides a dedicated generic MQTT-to-MariaDB sink that can be placed behind *MQTTIntegrator* when incoming vendor payloads should first be normalized.
...
* [MQTTStore](#mqttstore)
   ...
   * [Recommended Usage with MQTTIntegrator](#recommended-usage-with-mqttintegrator)
* [MQTTCli](#mqttcli)
...
**MQTTStore** is the generic persistence stage for the recommended production pipeline:

```text
MQTT devices -> MQTTBroker -> optional MQTTIntegrator normalization -> MQTTStore -> MariaDB
```

It intentionally stores MQTT data at the protocol-envelope level first, rather than assuming a vendor-specific payload such as The Things Network. Each received PUBLISH can be persisted with:

- connection/instance name,
- receive timestamp generated by MariaDB,
- topic,
- QoS,
- retain and duplicate flags,
- packet identifier,
- raw payload bytes,
- payload text when the message is valid UTF-8,
- parsed JSON payload when the message is valid JSON,
- truncation state when a configured payload-size cap is applied.

When `store --flatten-json true` is enabled, JSON scalar leaves are additionally projected into a generic field table using paths such as `device.temperature` or `rx_metadata[0].rssi`. This keeps raw replay/debug data and query-friendly scalar projections side by side without hard-coding device semantics in C++.
...
mqttstore in-mqtt --disabled=false \
  ...
  session --client-id mqttstore --retain-session \
  db --database mqttsuite --username mqttsuite --password secret \
  store --topic '#' \
        --raw-table mqtt_messages \
        --field-table mqtt_message_fields \
        --auto-create true \
        --flatten-json true
```
...
mqttstore in-wsmqtt --disabled=false \
  ...
  http --target /ws \
  session --client-id mqttstore-ws --retain-session \
  db --database mqttsuite --username mqttsuite --password secret \
  store --topic 'normalized/#'
```
...
| --- | --- | --- |
| `store --topic` | `#` | MQTT topic filters to persist. Append `##0`, `##1`, or `##2` to a filter to override subscription QoS. |
| `store --raw-table` | `mqtt_messages` | Raw envelope table. |
| `store --field-table` | `mqtt_message_fields` | Flattened JSON scalar table. |
| `store --auto-create` | `true` | Create generic tables automatically on startup. |
| `store --flatten-json` | `true` | Store JSON scalar leaves in the field table. |
| `store --max-payload-bytes` | `0` | Maximum stored payload size; `0` disables truncation. |

## Generated Tables

With `--auto-create true`, MQTTStore creates a raw envelope table similar to:
...
The optional field table stores generic scalar projections:

```sql
CREATE TABLE mqtt_message_fields (
  id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
...
## Recommended Usage with MQTTIntegrator

Use MQTTIntegrator for vendor-specific normalization and MQTTStore for persistence:

1. Devices publish vendor-specific data to the broker.
2. MQTTIntegrator maps device-specific topics/payloads into a canonical topic tree such as `normalized/<site>/<device>/<metric>`.
3. MQTTStore subscribes to `normalized/#` and persists every normalized event.

This keeps database persistence generic and lets normalization evolve independently through mapping JSON and plugins.
